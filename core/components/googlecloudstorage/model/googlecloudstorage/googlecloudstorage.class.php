...
class GoogleCloudStorage extends modMediaSource implements modMediaSourceInterface {
    ...
    public function getTypeName() {
        return $this->xpdo->lexicon('gcs.source_type');
    }
    ...
    public function getTypeDescription() {
        return $this->xpdo->lexicon('gcs.source_type_desc');
    }
        ...
        if (empty($this->driver)) {
            try {
                if (!file_exists(GCS_PRIMARY_KEY_FILE)) {
                    $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\nMissing the location of primary key file, given: " . GCS_PRIMARY_KEY_FILE, '', __METHOD__, __FILE__, __LINE__);
                    return false;
                }
                $cred = new Google_Auth_AssertionCredentials(
                        GCS_EMAIL_ADDRESS
                        , $this->scopes                             // leave this as property for run time changes
                        , file_get_contents(GCS_PRIMARY_KEY_FILE)
                );
    ...
    /**
     * Set the scope for the connection to Google Cloud Storage
     * @param array $scopes
     * @return void
     */
    ...
    public function createContainer($name, $parentContainer) {
        ...
        try {
            if ($this->driver->objects->get($this->bucket, $newPath)) {
                $err = $this->xpdo->lexicon('file_folder_err_ae') . ': ' . $newPath;
                $this->xpdo->error->message = $err;
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
                return false;
            }
        } catch (Exception $ex) {
            // leave this empty
        ...
        if (!$created) {
            $this->xpdo->error->message = $this->xpdo->lexicon('file_folder_err_create') . $newPath;
            $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n" . $ex->getMessage() . "\n" . $ex->getTraceAsString(), '', __METHOD__, __FILE__, __LINE__);
            return false;
        }
    ...
    public function removeContainer($path) {
        ...
        try {
            ...
            if (empty($objects)) {
                if (!$this->driver->objects->get($this->bucket, $path)) {
                    $err = $this->xpdo->lexicon('file_folder_err_ns') . ': ' . $path;
                    $this->xpdo->error->message = $err;
                    $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
                    return false;
                }
        ...
        try {
            if (empty($objects)) {
                $this->driver->objects->delete($this->bucket, $path);
            } else {
                $files = $objects->getItems();
                if (!empty($files)) {
                    foreach ($files as $file) {
    ...
    public function createObject($objectPath, $name, $content) {
        ...
        try {
            if ($this->driver->objects->get($this->bucket, $objectPath . $name, array('projection' => 'full'))) {
                $err = sprintf($this->xpdo->lexicon('file_err_ae'), $objectPath . $name);
        ...
        if (!$created) {
            $err = $this->xpdo->lexicon('file_err_create') . "\n" . $objectPath . $name;
            $this->addError('name', $err);
            $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
            return false;
        }
    ...
    public function updateObject($objectPath, $content) {
        ...
        if (!$updated) {
            $err = $this->xpdo->lexicon('file_err_update') . $objectPath;
            $this->addError('name', $err);
            $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
            return false;
        }
    ...
    public function removeObject($objectPath) {
        try {
            if (!$this->driver->objects->get($this->bucket, $objectPath)) {
                $err = $this->xpdo->lexicon('file_folder_err_ns') . ': ' . $objectPath;
                $this->xpdo->error->message = $err;
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
                return false;
            }
        ...
        try {
            ...
            $this->driver->objects->delete($this->bucket, $objectPath);
        } catch (Exception $ex) {
    ...
    public function renameObject($oldPath, $newName) {
        ...
        try {
            if (!$this->driver->objects->get($this->bucket, $oldPath)) {
                $err = $this->xpdo->lexicon('file_folder_err_ns') . ': ' . $oldPath;
        ...
        try {
            ...
            if (!$copied) {
                $err = $this->xpdo->lexicon('file_folder_err_rename') . ': ' . $oldPath;
                $this->addError('file', $err);
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "\n{$err}", '', __METHOD__, __FILE__, __LINE__);
                return false;
            }
    ...
    public function moveObject($from, $to, $point = 'append') {
        ...
        if (substr(strrev($from), 0, 1) == '/') {
            $this->xpdo->error->message = $this->xpdo->lexicon('gcs.no_move_folder', array(
                'from' => $from
            ));
            return $success;
        }
    ...
    public function getDefaultProperties() {
        return array(
            'gcs.app_name' => array(
                ...
                'options' => '',
                'value' => '',
                'lexicon' => 'googlecloudstorage:source',
            ),
            'gcs.client_id' => array(
                'name' => 'gcs.client_id',
            ...
            'gcs.default_scopes' => array(
                'name' => 'gcs.default_scopes',
                'desc' => 'prop_gcs.default_scopes_desc',
                'type' => 'textfield',
            ...
            'imageExtensions' => array(
                ...
                'desc' => 'prop_gcs.imageExtensions_desc',
                'type' => 'textfield',
            ...
            'thumbnailType' => array(
                'name' => 'thumbnailType',
                'desc' => 'prop_gcs.thumbnailType_desc',
                'type' => 'list',
                'options' => array(
            ...
            'thumbnailQuality' => array(
                ...
                'desc' => 'prop_gcs.thumbnailQuality_desc',
                'type' => 'textfield',
                'options' => '',
                'value' => 90,
                'lexicon' => 'googlecloudstorage:source',
